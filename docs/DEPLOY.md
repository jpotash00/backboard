...
| `OFFBOARD_SIGNUP_KEY` | optional | (unset → disabled) | Shared signup code for **self-serve** onboarding (`GET`/`POST /onboard`). Distinct from the admin key: it's **create-only** (a customer provisions itself, can't list/read/delete other tenants). Share a link `…/onboard?code=<this>`. Rotatable if leaked. Unset = `/onboard` returns 404. |
